...
scrapeExpiries <- function(symbol) {
  ...
  value.expiries <-
    xpathSApply(
      htmlParsed,
      "//div[@class = 'Fl(start) Pend(18px) option-contract-control drop-down-selector']//option",
      xmlGetAttr,
      "value"
    )
...
formatCalls <- function(calls) {
  ...
  for (callExp in calls) {
    relevantInfo <-
      cbind(callExp$strike, callExp$expiration, callExp$lastPrice)
...
formatPuts <- function(puts) {
  ...
  for (putExp in puts) {
    relevantInfo <-
      cbind(putExp$strike, putExp$expiration, putExp$lastPrice)
